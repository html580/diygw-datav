<template>
  <div class="diy-border-box-9" ref="borderRef">
...
const gradientId = computed(() => `border-box-9-gradient-${Math.random().toString(36).substr(2, 9)}`)
const maskId = computed(() => `border-box-9-mask-${Math.random().toString(36).substr(2, 9)}`)
...
.diy-border-box-9 .diy-border-svg-container {
  position: absolute;
  width: 100%;
  height: 100%;
  left: 0px;
...
.diy-border-box-9 .border-box-content {
  position: relative;
  width: 100%;
  height: 100%;
}
